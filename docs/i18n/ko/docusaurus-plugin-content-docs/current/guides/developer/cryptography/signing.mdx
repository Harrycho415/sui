...
Sui의 Move contract는 여러 signature scheme에 대한 on-chain 검증을 지원한다.
on-chain 검증에서 지원되는 모든 signature가 사용자 signature 검증에서 지원되는 것은 아니다.
transaction authorization에 유효한 signature scheme은 [Sui Signatures](/guides/developer/transactions/transaction-auth/auth-overview)를 참조한다.
